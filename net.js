...
var getHeader = function (req) {
  ...
  for (var i in req.headers) {
    if (
      !/^(host|connection|Access-|origin|referer|user-agent|user-doclever|path-doclever|url-doclever|method-doclever|headers-doclever|X-Requested-With)/i.test(
        i
      )
    ) {
      ret[i] = req.headers[i];
...
var filterResHeader = function (headers, res) {
  ...
  var resHeaders = res.getHeader("Access-Control-Expose-Headers")
...
function handleHeader(req, res) {
  ...
  res.setHeader(
    "Access-Control-Expose-Headers",
    "connection,content-length,date,x-powered-by,content-encoding,server,etag,accept-ranges,allow,content-language,set-cookie,doclever-request"
  );
  if (req.method == "OPTIONS") {
...
console.log(
  arguments.length > 0
    ? "鍐呯綉娴嬭瘯锛孧ock鏁版嵁姝ｇ洃鍚鍙ｏ細" + port
    : "姝ｅ湪杩愯涓紝璇风敤DOClever鐨勬帴鍙ｆ祴璇曢〉闈㈣繘琛屽唴缃戞祴璇曪紒"
);
